...
app = FastAPI(
    title="章悟式∞競輪OS KEIRIN.JP 3PDF API",
    version=VERSION,
    description=(
        "KEIRIN.JP公式3PDFから、しょーご式5点と"
        "市場残差3点を別々に計算します。"
    ),
)
...
async def analyze(
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="shogo-keirin-3pdf-") as tmp:
            root = Path(tmp)
            saved: dict[str, Path] = {}
...
<section class="card"><p><b>必要なのは全部KEIRIN.JPの3PDFだけ。</b>同じレースを入れてください。女子競輪は対象外です。</p>
<form id="form">
