...
app = FastAPI(
    title="章悟式∞競輪OS KEIRIN.JP 5PDF API",
    version=VERSION,
    description=(
        "KEIRIN.JP公式5PDFから、しょーご式5点と"
        "市場残差3点を別々に計算します。"
    ),
)
...
def health() -> dict[str, Any]:
    return {
        ...
        "model_status": "keirin_jp_5pdf_dual_strategy",
        "bet_type": "2車単",
        "race_type": "MEN_ONLY",
        "required_documents": [
...
async def analyze(
    ...
    recent_short_pdf: UploadFile = File(...),
    recent_detail_pdf: UploadFile = File(...),
    ...
    uploads = [
        ("basic", basic_pdf, "KEIRIN.JP 基本情報・並び予想PDF"),
        ("recent_short", recent_short_pdf, "KEIRIN.JP 直近成績①PDF"),
        ("recent_detail", recent_detail_pdf, "KEIRIN.JP 直近成績②PDF"),
        ("hs", hs_pdf, "KEIRIN.JP 着度数・H・S回数PDF"),
        ("odds", odds_pdf, "KEIRIN.JP 2車単オッズPDF"),
    ]
    try:
        with tempfile.TemporaryDirectory(prefix="shogo-keirin-5pdf-") as tmp:
            root = Path(tmp)
            saved: dict[str, Path] = {}
            for key, upload, label in uploads:
                path = _upload_path(root, upload, key)
                await _save_upload(upload, path, label)
                saved[key] = path
...
def backtest(request: BacktestRequest) -> dict[str, Any]:
    try:
        return evaluate_predictions(
            request.races, request.thresholds, request.stake_per_pair
        )
...
async def parse_result(
    result_pdf: UploadFile = File(...), pin: str = Form(default="")
) -> dict[str, Any]:
    _check_pin(pin)
...
<section class="card"><p><b>入力は全部KEIRIN.JP。</b>同じレースの5PDFを入れてください。女子競輪は対象外です。</p>
<form id="form">
<label>① 基本情報・並び予想PDF</label><input type="file" name="basic_pdf" accept="application/pdf" required>
<label>② 直近成績①PDF</label><input type="file" name="recent_short_pdf" accept="application/pdf" required>
<label>③ 直近成績②PDF</label><input type="file" name="recent_detail_pdf" accept="application/pdf" required>
