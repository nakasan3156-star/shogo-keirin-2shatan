#!/usr/bin/env python3
"""PR31固定 vs +「B取得4着以下で別線1着」前日補正を一発A/B評価する。

3つ目の補正量は2025H1の負けて強し分析で固定済みの +2.262pp を使用する。
2025年11-12月と2026年1-2月の結果から閾値・補正量を再調整しない。
運用ルールは3〜5点、3点未満は見送り。
"""
...
# 2025H1 discovery residual. Validation/external values are NOT used as the bonus size.
...
def apply_plus_j(frames, component, component_models, cats, used_base):
...
def calibrate_and_reliability(valid_market, other_markets):
    iso = IsotonicRegression(out_of_bounds="clip", y_min=1e-6, y_max=.95).fit(valid_market.pair_probability, valid_market.is_hit)
    for d in [valid_market, *other_markets]:
    ...
    return [baseline.apply_reliability(d, joint) for d in other_markets]


def model_path(name, component, pair_train, valid, test25, external, results25, odds25, races25, odds26, races26, results26, event_model, events, venue):
    maps = {}
    for key, frame in (("pair", pair_train), ("valid", valid), ("test", test25), ("external", external)):
        ...
        maps[key] = dict(zip(e.race_id, e.p_battle))
    pair_model = baseline.fit_pair_model(baseline.scenario_pair_rows(pair_train, maps["pair"], venue), results25)
    valid_pairs = baseline.predict_pairs(baseline.scenario_pair_rows(valid, maps["valid"], venue), pair_model)
    test_pairs = baseline.predict_pairs(baseline.scenario_pair_rows(test25, maps["test"], venue), pair_model)
    ext_pairs = baseline.predict_pairs(baseline.scenario_pair_rows(external, maps["external"], venue), pair_model)
    valid_market = baseline.attach_market(valid_pairs, odds25, races25, results25)
    test_market = baseline.attach_market(test_pairs, odds25, races25, results25)
    ext_market = baseline.attach_market(ext_pairs, odds26, races26, results26)
    test_market, ext_market = calibrate_and_reliability(valid_market, [test_market, ext_market])
    bets25, bets26 = production_bets(test_market), production_bets(ext_market)
    day2_ids25 = set(test25.loc[test25.day_no.gt(1), "race_id"])
    day2_ids26 = set(external.loc[external.day_no.gt(1), "race_id"])
    return {
        "name": name,
        "2025_test": metrics(bets25, int(test25.race_id.nunique())),
        "2025_test_day2plus": metrics(bets25[bets25.race_id.isin(day2_ids25)].copy(), len(day2_ids25)),
        "2026_external": metrics(bets26, int(external.race_id.nunique())),
...
def promotion_gate(base: dict, plus: dict) -> dict:
    ...
    base_stake = sum(base[p]["stake_yen"] for p in periods); plus_stake = sum(plus[p]["stake_yen"] for p in periods)
...
def main():
    ...
    riders26, results26, odds26 = riders26[riders26.race_id.isin(ids26)].copy(), results26[results26.race_id.isin(ids26)].copy(), odds26[odds26.race_id.isin(ids26)].copy()
    ...
    component_models, used_base = baseline.add_component_predictions([component_b, pair_b, valid_b, test_b, ext_b], component_b, cats)

    component_j, pair_j, valid_j, test_j, ext_j = [x.copy() for x in (component0, pair0, valid0, test0, ext0)]
    apply_plus_j([component_j, pair_j, valid_j, test_j, ext_j], component_j, component_models, cats, used_base)

    events = baseline.race_event_table(df)
    event_train = events[events.race_date.le(baseline.COMPONENT_END)]
    event_model = LogisticRegression(max_iter=500, class_weight="balanced", random_state=baseline.SEED).fit(event_train[baseline.EVENT_FEATURES].fillna(0), event_train.early_battle_label)
    venue, _ = baseline.venue_tables(component0)

    base_report, base25, base26 = model_path("PR31_FROZEN", component_b, pair_b, valid_b, test_b, ext_b, results25, odds25, races25, odds26, races26, results26, event_model, events, venue)
    plus_report, plus25, plus26 = model_path("PR31_PLUS_J", component_j, pair_j, valid_j, test_j, ext_j, results25, odds25, races25, odds26, races26, results26, event_model, events, venue)
    gate = promotion_gate(base_report, plus_report)

    j_counts = {}
    for name, frame in (("2025H1", component0), ("2025JulOct", pd.concat([pair0, valid0])), ("2025NovDec", test0), ("2026JanFeb", ext0)):
        m = frame.prior_J.eq(1)
        j_counts[name] = {"rows": int(m.sum()), "races": int(frame.loc[m, "race_id"].nunique())}

    report = {
        "execution": "Python actual run",
        "variant": "B取得4着以下で別線1着 only; PR31 core unchanged",
    ...
    pd.DataFrame([{"model": "base", "period": p, **base_report[p]} for p in ("2025_test", "2025_test_day2plus", "2026_external", "2026_external_day2plus")] + [{"model": "plus_j", "period": p, **plus_report[p]} for p in ("2025_test", "2025_test_day2plus", "2026_external", "2026_external_day2plus")]).to_csv(out / "metrics.csv", index=False, encoding="utf-8-sig")
    base25.to_csv(out / "base_bets_2025_test.csv", index=False); plus25.to_csv(out / "plus_j_bets_2025_test.csv", index=False)
    base26.to_csv(out / "base_bets_2026_external.csv", index=False); plus26.to_csv(out / "plus_j_bets_2026_external.csv", index=False)
