    ...
    def _predict_files(self, body: bytes) -> dict:
        ...
        for part in message.iter_parts():
            ...
            if name == "lambda_value":
                try:
                    lambda_value = float(
                        (part.get_payload(decode=True) or b"0.50").decode()
                    )
                except ValueError:
                    lambda_value = -1.0
                continue
            if name not in {*REQUIRED_UPLOADS, "ex_image"}:
                continue
            uploads[name] = (
                Path(part.get_filename() or name).name,
